...
const FieldedOutput = ({ output, loading }) => {
      ...
      {loading && !output ? (
        ...
        fields.map((f, i) => {
          ...
          return (
            <div
              key={i}
              className={`bg-gradient-to-br from-white to-slate-50 border rounded-lg overflow-hidden ${
...
const PlainOutput = ({ output, loading, onChange }) => {
  if (!output && !loading) return null;
  return (
    <div className="mt-5">
      ...
      <div className="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-lg relative overflow-hidden">
...
const CatchTab = () => {
  ...
  return (
    <div>
      ...
      <PlainOutput output={output} loading={loading} onChange={setOutput} />
    </div>
  );
};
...
function buildAirworkSectionPrompt(section, inputs, existingOutput) {
...
const AirworkOutputSection = ({ label, Icon, content, onCopy, copied, onRegenerate, isRegenerating, isLarge, isTitle }) => {
  return (
    <div>
...
/* Indeed 出力セクション */
const IndeedOutputSection = ({ label, Icon, content, onCopy, copied, isLarge, isTitle, isSummary, maxLength }) => {
...
const ImagePromptTab = ({ platform = "jobmedley" }) => {
  ...
  const run = async () => {
    ...
    try {
      ...
      const systemPrompt = `あなたはGemini・Nano Banana等の画像生成AI用プロンプト作成の専門家です。
ユーザー提供の求人情報と選択された条件から、高品質な画像を生成するための画像プロンプトを${count}パターン作成してください。

【掲載先プラットフォーム：${platformSpec.label}】
${platformSpec.promptInstruction}

【プロンプト作成のルール】
- ${language === "en" ? "英語で記述（画像生成AIは英語プロンプトで精度が高まる）" : "日本語で記述"}
- 各プロンプトは200〜400文字程度の詳細な描写
- 以下の要素を必ず含める：
  1. 被写体（人物の人数・性別・年代・表情・服装）
  2. シーン（背景・環境・小物）
  3. ライティング（明るさ・時間帯）
  4. スタイル（画像のテイスト）
  5. カメラアングル・構図
  6. 色調・雰囲気
  7. **画像仕様**（解像度・アスペクト比をプロンプト内に明記）
  8. ネガティブプロンプト（避けるべき要素）

【画像仕様の埋め込み（重要）】
プロンプト内に必ず以下を含めること：
- ${language === "en" ? `"high resolution, ${platformSpec.minWidth}x${platformSpec.minHeight} pixels or larger, aspect ratio ${platformSpec.recommendedRatio}, professional quality"` : `"高解像度、${platformSpec.minWidth}×${platformSpec.minHeight}px以上、アスペクト比${platformSpec.recommendedRatio}、プロ品質"`}
- 重要な被写体・テキストは画像の中央〜安全エリアに配置するよう指示
- ファイルサイズ${platformSpec.maxFileSize}を超えないよう、過度に複雑な装飾は避けるよう指示
...
出力は上記形式のみ。前置き・説明は不要。`;
      ...
      const sourceInfo =
        sourceMode === "manuscript"
          ? `【求人原稿・情報】\n${manuscript}`
          : `【業種】${scene.label}のみの情報で生成してください`;

      const userContent = `${sourceInfo}

【掲載先】
${platformSpec.label}（推奨サイズ：${platformSpec.minWidth}×${platformSpec.minHeight}px以上、最大${platformSpec.maxFileSize}）

【画像の条件】
- 業種シーン：${scene.label}（${scene.scene}）
- 画像スタイル：${style.label}（${style.detail}）
- 構図：${composition.label}（${composition.desc}）
- 雰囲気：${mood.label}（${mood.desc}）
- アスペクト比：${aspectRatio}
${additionalNotes ? `- 追加要望：${additionalNotes}` : ""}

上記条件を踏まえて、${platformSpec.label}に掲載するのに最適な高品質画像を生成するためのプロンプトを${count}パターン作成してください。
プロンプト内には必ず解像度（${platformSpec.minWidth}×${platformSpec.minHeight}px以上）とアスペクト比（${platformSpec.recommendedRatio}推奨）の指定を含めてください。`;

      const res = await callClaudeAPI(systemPrompt, userContent);
      setOutput(res);
    } catch (e) {
      setError("生成に失敗しました。時間をおいて再度お試しください。");
      console.error(e);
  ...
  return (
    <div>
      <div className="flex items-start gap-2 mb-4 p-3 bg-teal-50 border border-teal-200 rounded-md">
        <Info className="w-4 h-4 text-teal-600 mt-0.5 flex-shrink-0" />
        <div className="text-xs text-slate-700 leading-relaxed">
          求人原稿から<b className="text-teal-700">Gemini / Nano Banana用の画像生成プロンプト</b>を作成します。生成されたプロンプトをコピーして、Google AI StudioやGemini Webアプリに貼り付けて画像を生成してください。
        </div>
      </div>
      ...
      <div
        className={
          "mb-5 p-4 border rounded-lg " +
          (platform === "jobmedley" ? "bg-teal-50/50 border-teal-200" : "bg-rose-50/50 border-rose-200")
        }
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3
            className={
              "text-sm font-bold " + (platform === "jobmedley" ? "text-teal-800" : "text-rose-800")
            }
          >
            {platformSpec.label}の画像仕様
          </h3>
          <span
            className={
              "ml-auto text-[10px] font-mono px-2 py-0.5 rounded " +
              (platform === "jobmedley" ? "bg-teal-200 text-teal-800" : "bg-rose-200 text-rose-800")
            }
          >
            自動でプロンプトに反映
          </span>
        ...
        <ul className="space-y-1">
          {platformSpec.notes.map((note, i) => (
            <li key={i} className="text-xs text-slate-700 flex items-start gap-1.5">
              <span className={platform === "jobmedley" ? "text-teal-600" : "text-rose-600"}>※</span>
              <span>{note}</span>
            </li>
          ))}
        </ul>
      ...
      {/* Geminiツール案内 */}
      <div className="mb-5 p-4 bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 rounded-lg">
        <div className="flex items-center gap-2 mb-2">
          <ImageIcon className="w-4 h-4 text-blue-600" />
          <h3 className="text-sm font-bold text-slate-800">画像生成ツール</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {GEMINI_TOOLS.map((tool) => (
            <a
              ...
              className="group flex items-start gap-2 p-3 bg-white border border-slate-200 hover:border-blue-400 rounded-md transition"
            >
              <ExternalLink className="w-3.5 h-3.5 text-blue-600 mt-0.5 flex-shrink-0 group-hover:translate-x-0.5 transition" />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-slate-900">{tool.name}</div>
                <div className="text-[10px] text-slate-500 leading-relaxed mt-0.5">{tool.description}</div>
              </div>
            </a>
          ))}
        </div>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">入力ソース</label>
        </div>
        <div className="flex gap-2 bg-slate-100 border border-slate-200 rounded-md p-1 w-fit mb-3">
          <button
            onClick={() => setSourceMode("manuscript")}
            className={
              "flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded transition " +
              (sourceMode === "manuscript" ? "bg-teal-500 text-white" : "text-slate-600 hover:text-slate-800")
            }
          >
          ...
          <button
            ...
            className={
              "flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded transition " +
              (sourceMode === "scratch" ? "bg-teal-500 text-white" : "text-slate-600 hover:text-slate-800")
            }
          >
            <Plus className="w-3 h-3" />
            条件だけで生成
        ...
        {sourceMode === "manuscript" && (
          <textarea
            ...
            placeholder="求人原稿、事業所情報、キャッチコピーなどを貼り付け"
            className="w-full h-32 bg-white border border-slate-200 rounded-lg p-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-teal-500 focus:ring-1 ring-teal-200 transition resize-y"
          />
        )}
      </div>
      ...
      {/* 業種シーン */}
      <div className="mb-5">
        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">業種・シーン</label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {Object.entries(IMAGE_SCENE_TEMPLATES).map(([k, v]) => (
            <button
              key={k}
              onClick={() => setIndustryType(k)}
              className={
                "px-3 py-2 text-xs border rounded transition text-left " +
                (industryType === k ? "border-teal-500 bg-teal-50 text-slate-900 font-semibold" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300")
              }
            >
              {v.label}
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {IMAGE_STYLE_OPTIONS.map((s) => (
            <button
              ...
              className={
                "p-2.5 text-xs border rounded transition text-left " +
                (styleId === s.id ? "border-teal-500 bg-teal-50" : "border-slate-200 bg-white hover:border-slate-300")
              }
            >
              <div className="font-bold text-slate-900 mb-0.5">{s.label}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">構図</label>
          <select
            value={compositionId}
            onChange={(e) => setCompositionId(e.target.value)}
            className="w-full bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-teal-500"
          >
            {IMAGE_COMPOSITION_OPTIONS.map((c) => (
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">雰囲気</label>
          <select
            value={moodId}
            onChange={(e) => setMoodId(e.target.value)}
            className="w-full bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-teal-500"
          >
            {IMAGE_MOOD_OPTIONS.map((m) => (
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
            ...
            <span className="ml-1 text-[10px] text-slate-500 font-normal normal-case">
              （{platformSpec.label}推奨：{platformSpec.recommendedRatio}）
            </span>
          </label>
          <select
            value={aspectRatio}
          ...
          >
            <option value="16:9">
              16:9（横長・バナー向け{platform === "jobmedley" ? " ★推奨" : ""}）
            </option>
            <option value="4:3">
              4:3（横長・標準{platform === "airwork" ? " ★推奨" : ""}）
            </option>
            <option value="1:1">1:1（正方形・SNS向け）</option>
            <option value="3:4">3:4（縦長・ポスター）</option>
            <option value="9:16">9:16(縦長・ストーリー)</option>
          </select>
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">出力言語</label>
          <select
            value={language}
            onChange={(e) => setLanguage(e.target.value)}
            className="w-full bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-teal-500"
          >
            <option value="en">英語（推奨・精度高）</option>
            <option value="ja">日本語</option>
          </select>
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">パターン数</label>
          <select
            value={count}
            onChange={(e) => setCount(Number(e.target.value))}
            className="w-full bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-teal-500"
          >
            <option value={1}>1パターン</option>
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          placeholder="例：女性スタッフをメインに／施設ロゴを入れたい／季節は春／子供は小学生くらい..."
          className="w-full h-20 bg-white border border-slate-200 rounded-lg p-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-teal-500 focus:ring-1 ring-teal-200 transition resize-y"
        />
      </div>
          ...
          <div className="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-lg relative overflow-hidden">
            ...
            {loading && !output ? (
              ...
              <textarea
                ...
                className="w-full bg-transparent text-sm leading-[1.8] text-slate-900 p-5 focus:outline-none resize-y min-h-[400px]"
            ...
            <div className="mt-4 space-y-2">
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-md">
                <div className="flex items-start gap-2">
                  <AlertTriangle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" />
                  <div className="text-xs text-slate-700 leading-relaxed">
                    <b className="text-amber-700">使い方：</b>上記プロンプトをコピーして
                    <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" className="underline text-teal-600 hover:text-teal-700 mx-1">
                      Google AI Studio
                    </a>
                    や
                    <a href="https://gemini.google.com/" target="_blank" rel="noopener noreferrer" className="underline text-teal-600 hover:text-teal-700 mx-1">
                      Gemini
                    </a>
                    などに貼り付けて画像を生成してください。生成された画像はパターンごとに比較して、気に入ったものを採用してください。
                  </div>
                </div>
              </div>
              ...
              <div
                className={
                  ...
                  (platform === "jobmedley" ? "bg-teal-50 border-teal-200" : "bg-rose-50 border-rose-200")
                }
              >
                <div className="flex items-start gap-2">
                  <ImageIcon
                    className={
                      ...
                      (platform === "jobmedley" ? "text-teal-600" : "text-rose-600")
                    }
                  />
                  <div className="text-xs text-slate-700 leading-relaxed">
                    <b className={platform === "jobmedley" ? "text-teal-700" : "text-rose-700"}>
                      {platformSpec.label}にアップロード前のチェック：
                    </b>
                    生成された画像が以下の仕様を満たしているか確認してください。
